...
void vectorizedAddRow(Eliminator * elim, int rowToWhich, int rowWhich,
		      double mulFactor)
{
	int end = elim->left->cols - 1;
	//#pragma omp parallel for
	for (int k = 0; k < end; k += 2) {
		*(v2d *) get(*elim->left, rowToWhich, k) +=
		    *(v2d *) get(*elim->left, rowWhich,
				 k) * mulFactor;
	}
	if (elim->left->cols % 2) {
		*get(*elim->left, rowToWhich, elim->left->cols - 1) +=
		    *get(*elim->left, rowWhich,
			 elim->left->cols - 1) * mulFactor;
	}
	end = elim->right->cols - 1;
	//#pragma omp parallel for
	for (int k = 0; k < end; k += 2) {
		*(v2d *) get(*elim->right, rowToWhich, k) +=
		    *(v2d *) get(*elim->right, rowWhich,
				 k) * mulFactor;
	}
	if (elim->right->cols % 2) {
		*get(*elim->right, rowToWhich,
		     elim->right->cols - 1) +=
		    *get(*elim->right, rowWhich,
			 elim->right->cols - 1) * mulFactor;
	}
}
#endif
		      ...
		      double byHowMany)
{
	...
	for (int k = 0; k < end; k += 2) {
		...
		*(v2d *) get(*elim->left, rowWhich, k) *= byHowMany;
		//puts("megvan\n");
	}
	//printf("utsó elem\n");
	if (elim->left->cols % 2) {
		*get(*elim->left, rowWhich, elim->left->cols - 1) *=
		    byHowMany;
	}
	...
	//#pragma omp parallel for
	for (int k = 0; k < end; k += 2) {
		*(v2d *) get(*elim->right, rowWhich, k) *= byHowMany;
	}
	if (elim->right->cols % 2) {
		*get(*elim->right, rowWhich, elim->right->cols - 1) *=
		    byHowMany;
	}
}
#endif
...
#pragma omp parallel for
        ...
        // jobb oldalon lehet spórolni a normálásnál
#ifdef VECTORIZED
...
#pragma omp parallel for
    ...
    // a mátrix most normált
    // jön a felfelé eliminálás
...
#pragma omp parallel for
        ...
        // lehet spórolni a bal oldalon a normálással
#ifndef VECTORIZED

...
void inverz(Matrix mat, Matrix egyseg)
{
    ...
    for(k=matrix.sor-1; k>=0; k-=1)
    {
        ...
        for(int z=k-1; z>=0; z-=1)
        {
            pivot = matrix.adat[z*matrix.oszlop+sormax(matrix,z)];
            sorszorzas(matrix, z, 1.0/pivot);
            sorszorzas(egyseg, z, 1.0/pivot);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    for(meres = 0; meres < mat.sor; meres++)
    {
        ...
        for(valtozo = 0; valtozo < atoi(argv[1]); valtozo++)
        {
            ...
            {
                terv.adat[meres*terv.oszlop+valtozo*atoi(argv[2])+kitevo] = pow(mat.adat[meres*mat.oszlop+valtozo],kitevo)/* mat.adat[meres*mat.oszlop+mat.oszlop-1]*/;
            }
        }
    }
    Matrix b;
    ...
    {
        b.adat[bsor] = mat.adat[bsor*mat.oszlop+mat.oszlop-2]/*mat.adat[bsor*mat.oszlop+mat.oszlop-1]*/;
    ...
    for(i=0; i < megoldas.sor; ++i){
        megoldas.adat[i] *= pow(mat.adat[i*mat.oszlop+mat.oszlop-1],2);
    }
    ...
    for(i = 0; i < mat.sor; ++i)
    {
        double fuggvenyertek=0;
        for(g=0; g<megoldas.sor; ++g)
        {
            fuggvenyertek += megoldas.adat[g]*terv.adat[i*terv.oszlop+g];
        }

        fprintf(outp,"%f %f\n",fuggvenyertek,mat.adat[i*mat.oszlop+atoi(argv[1])]);
        khi += pow(mat.adat[i*mat.oszlop+atoi(argv[1])]-fuggvenyertek,2)/*/pow(mat.adat[i*mat.oszlop+atoi(argv[1])+1],2)*/;
    }
    printf("khi negyzet: %lf\n",khi);
    khi /= (mat.sor-terv.oszlop);
    printf("khi negyzet/ndf: %lf \n",khi);

    puts("kovariancia mátrix: \n");
    Matrixkiir(egyseg);
    puts("\n");
    Matrixkiir(megoldas);
    fclose(outp);
